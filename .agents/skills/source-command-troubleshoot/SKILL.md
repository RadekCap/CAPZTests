---
name: "source-command-troubleshoot"
description: "Migrated source command `troubleshoot`"
---

# source-command-troubleshoot

Use this skill when the user asks to run the migrated source command `troubleshoot`.
...
**Common Issues**:
- Tool not installed → Install from AGENTS.md prerequisites section
...
- Incorrect values → Check against AGENTS.md defaults
...
```bash
# Check resource group exists (name is ${WORKLOAD_CLUSTER_NAME}-resgroup, e.g., capz-tests-resgroup)
az group show --name <WORKLOAD_CLUSTER_NAME>-resgroup

# Check for conflicting resources
az resource list --resource-group <WORKLOAD_CLUSTER_NAME>-resgroup
...
**Related**:
- Reference to AGENTS.md section if applicable
...
4. Verify AGENTS.md is followed correctly
