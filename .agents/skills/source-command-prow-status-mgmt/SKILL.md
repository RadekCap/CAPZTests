---
name: "source-command-prow-status-mgmt"
description: "Migrated source command `prow-status-mgmt`"
---

# source-command-prow-status-mgmt

Use this skill when the user asks to run the migrated source command `prow-status-mgmt`.

## Command Template

Check the status of the Prow CI rehearsal for the Management Cluster PR (PR1) in openshift/release.
...
```bash
gh pr checks 76592 --repo openshift/release 2>&1 | grep "capi-tests/configure-prow-mgmt/mgmt-capz-e2e"
```
...
1. Check if `started.json` exists:
   ```bash
   curl -sL "https://storage.googleapis.com/test-platform-results/pr-logs/pull/openshift_release/76592/rehearse-76592-pull-ci-stolostron-capi-tests-configure-prow-mgmt-capz-e2e/<BUILD_ID>/started.json"
   ```
...
2. For running jobs, fetch live logs to determine step progress:
   ```bash
   curl -sL "https://prow.ci.openshift.org/log?container=test&id=<BUILD_ID>&job=rehearse-76592-pull-ci-stolostron-capi-tests-configure-prow-mgmt-capz-e2e" 2>&1 | grep -E "INFO.*Step|phase"
   ```
...
1. Fetch the junit XML for detailed results:
   ```bash
   curl -sL "https://storage.googleapis.com/test-platform-results/pr-logs/pull/openshift_release/76592/rehearse-76592-pull-ci-stolostron-capi-tests-configure-prow-mgmt-capz-e2e/<BUILD_ID>/artifacts/rehearse-76592-pull-ci-stolostron-capi-tests-configure-prow-mgmt-capz-e2e/mgmt-capz-e2e/artifacts/junit_operator.xml"
   ```
...
For running jobs, use live log lines like `Step mgmt-capz-e2e-X succeeded after Ys` to mark steps as PASSED, the last `Running step mgmt-capz-e2e-X` as RUNNING, and remaining steps as PENDING.
...
If `gh pr checks` shows no mgmt-capz-e2e check at all, suggest triggering one:
```bash
gh pr comment 76592 --repo openshift/release --body "/pj-rehearse pull-ci-stolostron-capi-tests-configure-prow-mgmt-capz-e2e"
```
